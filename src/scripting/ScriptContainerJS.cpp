...
void ScriptContainerJS::scriptThreadFunc(const ScriptInitOutputArgs& initOutput) {
    ...
    if(JS_IsException(ret)) {
        initOutput.error(std::string{"Script error: "} + getException());
        return;
    }
    auto runTypeProp = JS_GetPropertyStr(mJSContext, ret, "runType");
    util::DestructWrapper destructRunTypeProp{[&]{ JS_FreeValue(mJSContext, runTypeProp); }};
    if(!JS_IsString(runTypeProp)) {
        initOutput.error("Init return runType is not a string");
        return;
    }
    auto runTypePropStr = JS_ToCString(mJSContext, runTypeProp);
    util::DestructWrapper destructRunTypePropStr{[&]{ JS_FreeCString(mJSContext, runTypePropStr); }};
    if(strcmp(runTypePropStr, "sync") == 0) {
        mScriptInitReturn.runType = ScriptRunType::Sync;
    } else if(strcmp(runTypePropStr, "async") == 0) {
        mScriptInitReturn.runType = ScriptRunType::Async;
    } else {
        initOutput.error(std::string{"Init return runType should be 'sync' or 'async', not '"} + runTypePropStr + "'");
        return;
    }
    destructRunTypePropStr.execute();
    destructRunTypeProp.execute();
    destructRet.execute();
    ...
    while(!mShouldAbort) {
        ...
        auto getStringProperty = [this] (auto obj, std::string_view field) -> std::optional<std::string> {
            auto prop = JS_GetPropertyStr(mJSContext, obj, field.data());
        ...
        if(!JS_IsArray(mJSContext, runResult)) {
            if(JS_IsException(runResult)) {
                output.error(getException());
                continue;
            }
        ...
        uint32_t i = 0;
        while(true) {
            auto action = JS_GetPropertyUint32(mJSContext, runResult, i);
            ...
            auto maybeActionTypeStr = getStringProperty(action, "type");
            ...
            if(actionType == "publish") {
                auto topic = getStringProperty(action, "topic");
                if(!topic) {
                ...
                auto payloadStr = getStringProperty(action, "payloadStr");
                if(payloadStr) {
                ...
                output.publish(*topic, std::move(payload), qos, retain);
            }
            i += 1;
        }
